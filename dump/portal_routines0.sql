...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `clients_master_delete_client` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `clients_master_delete_client`(
	in client_id int
)
BEGIN

	update clients_master cm
    set cm.is_active = 0
    where cm.id = client_id;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `clients_master_edit_client` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `clients_master_edit_client`(
	in client_id int,
    in name varchar(45),
    in cin varchar(21),
    in type_id int,
...
BEGIN
    ...
    set cm.name = `name`,
		cm.cin = cin,
        cm.type_id = type_id,
        cm.ca_firm_name = ca_firm_name,
        cm.ca_address = ca_address,
        cm.ca_pan = ca_pan,
        cm.ca_email = ca_email,
        cm.con_name = con_name,
        cm.con_email = con_email,
        cm.con_phone = con_phone
	where cm.id = client_id;
...
BEGIN
	if off_set is null 
	   ...
	   records_per_page is null then  
		set off_set = 0;
        set records_per_page = (select count(*) from clients_master);
	end if;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `roles_delete_role`(
	in role_id int
)
BEGIN
	update roles r
...
BEGIN
    ...
    declare i int default 0; -- loop control
    
    declare sub_task json;
    
    declare description varchar(150);
    declare cost int;
    declare comments varchar(300);
    declare status_id int default null;
    declare assigned_to int default null;
    
    declare sub_task_master_id int;
    declare sub_task_id int;
    
    
    while i < json_length(sub_tasks_arr) do
    
		select json_extract(sub_tasks_arr, concat('$[', i, ']'))
		into sub_task;

		select sub_task;
        
        select json_unquote(json_extract(sub_task, concat('$.description'))) into description;
		select json_extract(sub_task, concat('$.cost')) into cost;
	    select json_unquote(json_extract(sub_task, concat('$.comments'))) into comments;
	    select json_extract(sub_task, concat('$.assignedTo')) into assigned_to;
        select json_extract(sub_task, concat('$.statusId')) into status_id;
	
		select description, cost, comments, assigned_to;
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `sub_tasks_edit` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `sub_tasks_edit`(
	in task_id int,
	in sub_task_arr json
)
BEGIN

	declare max_iterations int default 200; -- security
	declare i int default 0;	-- loop control
	declare sub_task_id int; 	-- for loop
    declare sub_task json; 		-- for loop
    declare exist int;
    declare status_id int default null;
    declare assigned_to int default null;
	declare description varchar(150);
	...
	while i < json_length(sub_task_arr) && i < max_iterations do
               
        -- get id of sub_task
		select json_extract(sub_task_arr, concat('$[', i, ']')) 
        into sub_task;
        
        
        select 
			json_extract(sub_task, concat('$.statusId')) 
        into status_id;
        
        if status_id is null then 
			-- setting status_id = 2 ('to do')
            set status_id = 2;
		end if;
        
        
		select 
			json_extract(sub_task, concat('$.assignedTo')) 
        into assigned_to;
        
        
        select 
			json_unquote(json_extract(sub_task, concat('$.description'))) 
        into description;
        
        
        select 
			json_extract(sub_task, concat('$.cost')) 
        into cost;
        
        
        select 
			json_unquote(json_extract(sub_task, concat('$.comments'))) 
        into comments;
        

        select 
			json_extract(sub_task, concat('$.id')) 
        into sub_task_id;
            
	
        
        -- check if sub_task with given id already exist
        select st.id
        from 
			sub_tasks st
		where st.id = sub_task_id
        into exist;
	

        if exist is not null then
            
			-- sub_task exist so update its details
            update sub_tasks st
            set st.assigned_to = assigned_to,
			...
			-- sub_task don't exist so create new one
			insert into sub_tasks
				(
					task_id,
					assigned_to,
...
BEGIN
	...
	-- if `save...` is selected we have to 
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `tasks_edit_task` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `tasks_edit_task`(
	in task_id int,
	in tasks_master_id int,
	in title varchar(100), 
    in description varchar(300),
...
BEGIN
    ...
    -- update general details of task
		update tasks t
		set t.id = task_id,
			t.tasks_master_id = tasks_master_id,
			t.title = title,
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `tasks_master_edit_task` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `tasks_master_edit_task`(
	in task_master_id int,
	in title varchar(100), 
    in description varchar(300),
	in cost int
...
/*!50003 SET character_set_results = @saved_cs_results */ ;
/*!50003 SET collation_connection  = @saved_col_connection */ ;
/*!50003 DROP PROCEDURE IF EXISTS `users_delete_user` */;
/*!50003 SET @saved_cs_client      = @@character_set_client */ ;
/*!50003 SET @saved_cs_results     = @@character_set_results */ ;
/*!50003 SET @saved_col_connection = @@collation_connection */ ;
/*!50003 SET character_set_client  = utf8mb4 */ ;
...
/*!50003 SET sql_mode              = 'STRICT_TRANS_TABLES,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `users_delete_user`(
	in user_id int
)
BEGIN
...
-- Dump completed on 2022-12-07 16:18:55
